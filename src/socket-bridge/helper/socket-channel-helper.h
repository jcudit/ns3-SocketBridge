...
#ifndef SOCKET_CHANNEL_HELPER_H
#define SOCKET_CHANNEL_HELPER_H
 ...
 */
class SocketChannelHelper
{
public:
  /**
   * Construct a SocketChannelHelper to make life easier for people wanting to 
   ...
   *
   * \returns A pointer to the new SocketChannel
   */
  Ptr<SocketChannel> Create ();
   ...
   * to the channel.
   *
   * \param bridge The Ptr<SocketBridge> to install to the new channel
   * \returns A pointer to the new SocketChannel
   */
  Ptr<SocketChannel> Create (Ptr<SocketBridge> bridge);

  /**
   * This method attaches an existing SocketBridge to an existing channel.
   *
   * \param channel The Ptr<SocketChannel> that the bridge will be connected to
   * \param bridge The Ptr<SocketBridge> to install to the new channel
   */
  void Install (Ptr<SocketChannel> channel, Ptr<SocketBridge> bridge);
